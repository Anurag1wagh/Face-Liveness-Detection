from tkinter import * #Represents all function and built in module in tikenter library.
...
root.title("Security Online Banking System")
...
lbl = tk.Label(root, text="Security Online Banking System", font=('times', 40,' bold '), height=1, width=30,bg="Black",fg="indian red")
lbl.place(x=330, y=5)
...
frame_display = tk.LabelFrame(root, text=" --Display-- ", width=900, height=250, bd=5, font=('times', 14, ' bold '),bg="#FFDFDD")
...
#           )
        for(x,y,w,h) in faces:
            ...
            if (confidence < 50):
                ...
                f.close()
                cam.release()
                update_label('Authenticated User...with face recognization')
                button1 = tk.Button(root, text="Check Liveness", command=live,width=20, height=1, font=('times', 15, ' bold '),bg="purple",fg="white")
...
button3 = tk.Button(frame_alpr, text="Authenticate User", command=Test_database, width=20, height=1, font=('times', 15, ' bold '),bg="purple",fg="white")
button3.place(x=10, y=220)

entry2=tk.Entry(frame_alpr,bd=2,width=7)
entry2.place(x=210, y=110)

button4 = tk.Button(frame_alpr, text="Display All Records", command=ID,width=20, height=1,bg="purple",fg="white", font=('times', 15, ' bold '))
